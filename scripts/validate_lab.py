#!/usr/bin/env python3
"""Validate metadata and files for every runnable lab in this repository."""
...
def main() -> int:
    ...
    algorithms = load_json(ROOT / "data" / "lab-algorithms.json")
    ...
    for experiment in experiments:
        ...
        if experiment.get("status") == "available":
            ...
            for link in experiment.get("main_garden_links", []):
                link_path = Path(urlsplit(link).path)
                is_garden_link = (
                    isinstance(link, str)
                    and not link_path.is_absolute()
                    and ".." not in link_path.parts
                )
                require(
                    is_garden_link,
                    f"{eid}: garden link should stay repository-local: {link}",
                    errors,
                )
                if is_garden_link:
                    garden_path = ROOT / link_path
                    require(
                        garden_path.is_file(),
                        f"{eid}: garden link target not found: {link}",
                        errors,
                    )
