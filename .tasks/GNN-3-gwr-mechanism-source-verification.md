...
- [ ] For every supported entry, record an evidence status (`directly verified` or `abstract-supported`), a concise paraphrase, and a pinpoint locator such as page, section, equation, figure, or algorithm step. For every unresolved entry, record the source locations checked and why the evidence was insufficient; do not infer missing mechanics.
...
- [ ] If the private source cannot be read reliably, source identity cannot be confirmed, or a mechanism detail cannot be represented as a faithful short paraphrase without resolving source ambiguity, stop without completing the task and report the exact blocker for human review.
